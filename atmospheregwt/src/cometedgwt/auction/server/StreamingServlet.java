 ...
 *
 */
package cometedgwt.auction.server;
...
public class StreamingServlet extends RemoteServiceServlet {
	...
	private final static byte[] JUNK = ("<!-- Comet is a programming technique that enables web " + "servers to send data to the client without having any need " + "for the client to request it. -->\n").getBytes();
	...
	private void sendKeepAlive(OutputStream out) throws IOException {
		writeToStream(out, writeCallback(new StringBuffer(), "keepAliveInternal", Integer.toString(counter++)).toString());
	}

	private StringBuffer writeCallback(StringBuffer stream, String topicName, String data) throws UnsupportedEncodingException {
